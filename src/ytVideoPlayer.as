package
{
	...
	[AccessibilityClass(implementation="ytPlayerAccImp")]
	 ...
	 *  @productversion Flex 4
	 */
	public class ytVideoPlayer extends SkinnableComponent
	{
		 ...
		 *  @productversion Flex 4
		 */
		public var videoDisplay:youTube;
		...
		public function get volume():Number
		{
			if (videoDisplay)
			{
				return videoDisplay.volume /100;
			}
			else
			{
		...
		{ 
			if (ytVideoPlayer.createAccessibilityImplementation != null)
				ytVideoPlayer.createAccessibilityImplementation(this);
		}
		...
		{   
			var state:String = videoDisplay.mediaPlayerState;
		...
		private function fullScreenButton_clickHandler(event:MouseEvent):void
		{
			if (!fullScreen)
			{
				...
				beforeFullScreenInfo = {parent: this.parent,
					x: this.x,
					y: this.y,
					explicitWidth: this.width,
					explicitHeight: this.height};
				...
				videoDisplay.resize(screenBounds.width, screenBounds.height);
			}
			else
			{
		...
		//	
			...
			videoDisplay.resize(this.explicitWidth-2 ,this.explicitHeight-25);
