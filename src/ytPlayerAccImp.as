package
{
	 ...
	 */
	public class ytPlayerAccImp extends AccImpl
	{
		...
		{
			ytVideoPlayer.createAccessibilityImplementation =
		 ...
		 *  @productversion Flex 4
		 */
		public function ytPlayerAccImp(master:UIComponent)
		{
			super(master);
			...
			var playPauseButton:ToggleButtonBase =
				ytVideoPlayer(master).playPauseButton;
			if (playPauseButton)
			{
				playPauseButton.addEventListener(Event.CHANGE, eventHandler);
				if (ytVideoPlayer(master).tabIndex > 0 &&
					playPauseButton.tabIndex == -1)
					playPauseButton.tabIndex = ytVideoPlayer(master).tabIndex;
				
			}
			
			var volumeBar:VolumeBar = ytVideoPlayer(master).volumeBar;
			if (volumeBar)
			{
				...
				if (ytVideoPlayer(master).tabIndex > 0 &&
					...
					volumeBar.tabIndex = ytVideoPlayer(master).tabIndex;
			}
			
			if (ytVideoPlayer(master).scrubBar)    
				if (ytVideoPlayer(master).tabIndex > 0 &&
					ytVideoPlayer(master).scrubBar.tabIndex == -1)
					ytVideoPlayer(master).scrubBar.tabIndex = ytVideoPlayer(master).tabIndex;
			
			if (ytVideoPlayer(master).fullScreenButton)
				if (ytVideoPlayer(master).tabIndex > 0 &&
					ytVideoPlayer(master).fullScreenButton.tabIndex == -1)
					ytVideoPlayer(master).fullScreenButton.tabIndex = ytVideoPlayer(master).tabIndex;
		}
		...
		{
			var videoPlayer:ytVideoPlayer = ytVideoPlayer(master);
		...
		override public function get_accRole(childID:uint):uint
		{
			...
			switch (childID) 
			{
				...
				case VIDEOPLAYER_PLAYPAUSEBUTTON:
				{
					if (!ytVideoPlayer(master).playPauseButton)
						break;
				...
				case VIDEOPLAYER_CURRENTTIMEDISPLAY:
				{
					if (!ytVideoPlayer(master).currentTimeDisplay)
						break;
				...
				case VIDEOPLAYER_MUTEBUTTON:
				{
					if (!ytVideoPlayer(master).volumeBar)
						break;
		...
		override protected function eventHandler(event:Event):void
		{
			...
			switch (event.type)
			{
				...
				{
					playPauseButton = ytVideoPlayer(master).playPauseButton;
					...
					volumeBar = ytVideoPlayer(master).volumeBar;
					...
					scrubBar = ytVideoPlayer(master).scrubBar;
					if (SkinPartEvent(event).instance == scrubBar)
					{
						if (ytVideoPlayer(master).tabIndex > 0 &&
							scrubBar.tabIndex == -1)
							scrubBar.tabIndex = ytVideoPlayer(master).tabIndex;
					}
					
					fullScreenButton = ytVideoPlayer(master).fullScreenButton;
					if (SkinPartEvent(event).instance == fullScreenButton)
					{
						if (ytVideoPlayer(master).tabIndex > 0 &&
							fullScreenButton.tabIndex == -1)
							fullScreenButton.tabIndex = ytVideoPlayer(master).tabIndex;
					}
